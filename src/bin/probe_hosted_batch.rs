...
fn main() -> Result<()> {
    ...
    match args.command {
        Command::Preprocess { artifact_ids } => run_preprocess(
            &*factory,
            &read_store,
            &artifact_ids,
        ...
        Command::Extract { artifact_ids } => run_extract(
            &*factory,
            &read_store,
            &artifact_ids,
            Duration::from_secs(args.poll_seconds),
        ...
        Command::Reconcile { job_ids } => run_reconcile(
            &*factory,
            &postgres,
            &read_store,
            &derived_store,
...
) -> Result<(Box<dyn ArtifactProcessorFactory>, String)> {
    match provider {
        HostedProvider::OpenAi => {
            ...
            let model = config.standard_model.clone();
...
fn run_preprocess(
    ...
    let submitter = factory
        ...
        .ok_or_else(|| anyhow!("preprocess submitter unavailable"))?;
...
fn run_extract(
    ...
    let submitter = factory
        ...
        .ok_or_else(|| anyhow!("extraction submitter unavailable"))?;
    let mut inputs = Vec::new();
    for artifact_id in artifact_ids {
        let loaded = read_store
            .load_artifact_for_enrichment(artifact_id)?
...
fn run_reconcile(
    ...
    let submitter = factory
        ...
        .ok_or_else(|| anyhow!("reconciliation submitter unavailable"))?;
    ...
    for input in &inputs {
        println!(
            "  {} | candidates={}",
            input.artifact_id,
            input.memories.len() + input.relationships.len()
        );
    }
    let handle = submitter.prepare_and_submit(&inputs)?;
    println!("Batch: {}", handle.batch_id);
...
impl HostedProvider {
    fn from_str(value: &str) -> Result<Self> {
        match value {
            ...
            "grok" => Ok(Self::Grok),
            _ => Err(anyhow!(
                "unsupported provider {value}; expected openai, anthropic, claude, or grok"
            )),
        }
    }
